...
describe BarmanCheck::Parser do
  describe 'barman reports all OK' do
    let :check_data do
      ['Server main:', 'ssh: OK ', 'PostgreSQL: OK ',
       'archive_mode: OK ',
       'archive_command:  OK ',
    ...
    let :list_data do
      ['main 20160119T000001 - Tue Jan 19 00:00:49 2016 - Size: 28.2 GiB - WAL Size: 33.3 MiB',
    ...
    let :parser do
      described_class.new(check_data, list_data)
    end
  ...
  describe 'barman reports most recent backup failures' do
    let :check_data do
      ['Server main:', 'ssh: FAILED (return code: 255) ', 'PostgreSQL: OK ',
       'archive_mode: OK ',
       'archive_command:  OK ',
       'continuous archiving: OK ',
       'directories: OK ',
  ...
  describe 'barman reports backups too old and not growing' do
    ...
    let :list_data do
      ['main 20160119T000001 - Tue Jan 26 10:15:49 2016 - Size: 22.2 GiB - WAL Size: 33.3 MiB',
       'main 20160118T000001 - Mon Jan 18 00:00:43 2016 - Size: 27.9 GiB - WAL Size: 102.3 MiB',
       'main 20160117T000002 - Sun Jan 17 00:00:36 2016 - Size: 27.0 GiB - WAL Size: 68.7 MiB']
    end
